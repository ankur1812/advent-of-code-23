import { puzzle } from "~/puzzles/day2";
...
`,
  ...
  games.forEach((g, index) => {
    let sets = g?.split(": ")[1]?.split("; ");

    let validGame = sets?.every((s) => {
      let balls = s?.split(", ");
      return balls?.every((b) => {
        let [count, color] = b.split(" ");
        return max[color] >= parseInt(count);
      });
    });
    if (validGame) {
      sum += index + 1;
...
`,
  ...
  let sum = 0;
  games.forEach((g, index) => {
    const minHash = { red: 0, green: 0, blue: 0 };
    let sets = g?.split(": ")[1]?.split("; ");
    sets?.forEach((s) => {
      let balls = s?.split(", ");
      balls?.forEach((b) => {
        let [count, color] = b.split(" ");
        if(minHash[color] < parseInt(count)) minHash[color] = parseInt(count)
      });
    });
    let gamePowerSet = 1;
    ['red', 'green', 'blue'].forEach( c => {
      gamePowerSet *= minHash[c]
    })
      sum += gamePowerSet;
  });
  return sum;
...
<p>Your puzzle answer was <code>2331</code>.</p><article class="day-desc"><h2 id="part2">--- Part Two ---</h2><p>The Elf says they've stopped producing snow because they aren't getting any <em>water</em>! He isn't sure why the water stopped; however, he can show you how to get to the water source to check it out for yourself. It's just up ahead!</p>
...
<p>Your puzzle answer was <code>71585</code>.</p><p class="day-success">Both parts of this puzzle are complete! They provide two gold stars: **</p>
<p>At this point, you should <a href="/2023">return to your Advent calendar</a> and try another puzzle.</p>
<p>If you still want to see it, you can <a href="api/day/2" target="_blank">get your puzzle input</a>.</p>
